...
function showManualJoinLink(joinUrl) {
  ...
  if (copyStatus) {
    ...
    instruction.textContent = "Link zum manuellen Kopieren:";
    ...
    copyStatus.appendChild(instruction);
    copyStatus.appendChild(link);
  }
}
...
async function copyJoinLink() {
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title: "Harnelyzer Best Fit – Test beitreten",
        text: `Tritt dem 3-Kamera-Test ${state.testId} bei.`,
        url: joinUrl
      });

      if (copyStatus) {
        copyStatus.textContent =
          "Teilen-Menü geöffnet. Sende den Link an Handy B oder C.";
      }
      ...
      if (error && error.name === "AbortError") {
        if (copyStatus) {
          copyStatus.textContent = "Teilen abgebrochen. Du kannst es erneut versuchen.";
        }
  ...
  if (navigator.clipboard && window.isSecureContext) {
    try {
      ...
      if (copyStatus) {
        copyStatus.textContent =
          "✓ Beitritts-Link kopiert. Öffne WhatsApp und füge ihn bei Handy B oder C ein.";
      }
...
function readJoinUrl() {
  ...
  if (manualInput) manualInput.value = testId;
  if (roleInput) roleInput.value = "B";
...
async function uploadRecordedVideo(blob) {
  ...
  if (!state.supabase) throw new Error("Supabase-SDK nicht geladen.");
  if (!state.testId) throw new Error("Keine Test-ID vorhanden.");
...
function toggleRecording() {
  if (state.cameraRecording) stopRecordingIfActive();
  else startRecording();
}
...
async function broadcast(type, payload = {}) {
  if (!state.channel || !state.realtimeConnected) return;
