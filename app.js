...
function roleShortName(role) {
  const roles = { A: "KAMERA A", B: "KAMERA B", C: "KAMERA C" };
  return roles[role] || roles.A;
}
...
function saveSession() {
  safeStorageSet("bestFitSession", JSON.stringify({
    testId: state.testId,
    role: state.role,
...
function updateSessionUi() {
  const sessionText = state.testId ? `${state.testId} · ${roleShortName(state.role)}` : "KEIN TEST AKTIV";
  ...
  if (recordTestId) recordTestId.textContent = `${state.testId || "BF-000000"} · ${roleName(state.role)}`;
  if (resultTestId) resultTestId.textContent = state.testId || "BF-000000";
  if (resultDogName) resultDogName.textContent = state.dogName ? state.dogName.toUpperCase() : "DEIN HUND";
...
function readJoinUrl() {
  ...
  if (joinHint) joinHint.textContent = "Die Test-ID wurde aus dem QR-Code übernommen. Wähle jetzt die Rolle dieses Handys.";
...
function renderSetup() {
  const positions = {
    A: { title: "Kamera A seitlich aufstellen", text: "Stelle dieses Handy seitlich an der Laufstrecke auf. Der ganze Hund muss im Bild sein.", letter: "A", direction: "SEITLICH", left: "48%", right: "auto", top: "auto", bottom: "20px" },
    B: { title: "Kamera B vorne links aufstellen", text: "Stelle dieses Handy vorne links an der Strecke auf. Der Hund läuft gerade auf die Kamera zu.", letter: "B", direction: "VORNE LINKS", left: "11%", right: "auto", top: "25%", bottom: "auto" },
    C: { title: "Kamera C hinten rechts aufstellen", text: "Stelle dieses Handy hinter der Strecke auf. Der Hund läuft gerade von der Kamera weg.", letter: "C", direction: "HINTEN RECHTS", left: "auto", right: "10%", top: "25%", bottom: "auto" }
  };
...
function updateFrameRateSelection() {
  ...
  setCameraActualSettings(`Gewählt: ${state.selectedFrameRate} fps. Die echte Bildrate wird nach der Kamera-Freigabe angezeigt.`);
  saveSession();
}
...
function openCameraDialog() {
  ...
  if (!cameraSupported()) {
    ...
    setCameraStatus("Kamera nicht verfügbar: Öffne die Website in Chrome über HTTPS, nicht in einer eingebetteten Vorschau.", "error");
    return;
  }

  if (enableButton) enableButton.disabled = false;
  setCameraStatus("Wähle die Bildrate und tippe auf „KAMERA AKTIVIEREN“.");
}
...
async function enableCamera() {
  if (!cameraSupported()) {
    setCameraStatus("Kamera nicht verfügbar: Bitte in Chrome über HTTPS öffnen.", "error");
    return;
  }
  ...
  try {
    setCameraStatus(`Kamera wird mit Wunschwert ${state.selectedFrameRate} fps aktiviert ...`);
    ...
    setCameraActualSettings(`AKTIV: ${width} × ${height} · ${realFps} fps · angefordert: ${state.selectedFrameRate} fps`);

    const enableButton = byId("btnCameraEnable");
    const recordButton = byId("btnCameraRecord");

    if (enableButton) enableButton.textContent = "KAMERA AKTIV";
    if (recordButton) recordButton.disabled = !window.MediaRecorder;
    ...
    const messages = {
      NotAllowedError: "Kamera nicht freigegeben. Tippe auf das Schloss-Symbol neben der Adresse und erlaube Kamera.",
      SecurityError: "Kamera wird durch die aktuelle Umgebung blockiert. Öffne die GitHub-Pages-Adresse direkt in Chrome.",
      NotFoundError: "Keine Kamera gefunden.",
      NotReadableError: "Die Kamera wird gerade von einer anderen App verwendet. Schließe Kamera, WhatsApp oder Instagram und versuche es erneut.",
      OverconstrainedError: "Diese Kamera unterstützt die gewählte Einstellung nicht. Wähle 30 fps und versuche es erneut."
    };

    setCameraStatus(messages[errorName] || `Kamera konnte nicht gestartet werden: ${errorName}.`, "error");
  }
}
...
function startRecording() {
  ...
  if (!window.MediaRecorder) {
    setCameraStatus("Lokale Videoaufnahme wird von diesem Browser nicht unterstützt.", "error");
    return;
  }
  ...
  try {
    ...
    state.mediaRecorder = mimeType ? new MediaRecorder(state.cameraStream, { mimeType }) : new MediaRecorder(state.cameraStream);
    ...
    state.mediaRecorder.addEventListener("stop", () => {
      ...
      const recordButton = byId("btnCameraRecord");
      if (recordButton) recordButton.textContent = "AUFNAHME STARTEN";
      ...
      setCameraStatus("Aufnahme beendet. Prüfe den Clip oder verwirf ihn für eine neue Aufnahme.");
      setRecordStatus(`Clip lokal gespeichert: ${state.testId} · ${roleName(state.role)} · Wunschwert ${state.selectedFrameRate} fps.`);
    ...
    const recordButton = byId("btnCameraRecord");
    if (recordButton) recordButton.textContent = "AUFNAHME BEENDEN";

    setCameraStatus("AUFZEICHNUNG LÄUFT · Erst sichtbar klatschen, dann zwei gerade Laufsequenzen filmen.", "recording");
  } catch (error) {
    setCameraStatus(`Aufnahme konnte nicht gestartet werden: ${error?.name || "unbekannter Fehler"}.`, "error");
  }
}
...
function discardRecording() {
  resetPlayback();
  const preview = byId("cameraPreview");
  if (state.cameraStream && preview) preview.hidden = false;
  setCameraStatus("Aufnahme verworfen. Kamera ist bereit für einen neuen Clip.");
}
...
async function copyJoinLink() {
  ...
  try {
    ...
    if (copyStatus) copyStatus.textContent = "Beitritts-Link kopiert. Sende ihn per WhatsApp an Handy B und C.";
  } catch (error) {
    if (copyStatus) copyStatus.textContent = `Link kopieren und weitergeben: ${joinUrl}`;
  }
}

function ensureSupabaseClient() {
  if (state.supabase || !window.supabase?.createClient) return;
...
function readyText() {
  ...
  return peers ? `Bereit: ${peers}` : "Noch keine Kamera bereit.";
}
...
function runCountdown(startAt) {
  ...
  const tick = () => {
    ...
    if (remainingMs <= 0) {
      clearCountdown();
      if (!state.cameraRecording) startRecording();
      return;
    }
...
async function broadcast(type, payload = {}) {
  ...
  await state.channel.send({ type: "broadcast", event: type, payload });
}

function handleRealtimeEvent(event, payload) {
  if (!payload || payload.testId !== state.testId) return;
  ...
  if (event === "stop") {
    clearCountdown();
    stopRecordingIfActive();
    setCameraStatus("Master-Stopp empfangen.");
  }
}
...
function connectRealtime() {
  ...
  if (!state.supabase) {
    setRecordStatus("Supabase-SDK fehlt. Prüfe die Script-Zeile in index.html.");
    return;
  }
  ...
  state.channel.on("broadcast", { event: "ready" }, ({ payload }) => handleRealtimeEvent("ready", payload));
  state.channel.on("broadcast", { event: "start" }, ({ payload }) => handleRealtimeEvent("start", payload));
  state.channel.on("broadcast", { event: "stop" }, ({ payload }) => handleRealtimeEvent("stop", payload));

  state.channel.subscribe((status) => {
    if (status === "SUBSCRIBED") {
      setRecordStatus(`Live verbunden · ${state.testId} · ${roleName(state.role)}`);
    }
  });
}
...
async function sendReady() {
  ...
  setCameraStatus("Diese Kamera ist bereit und wartet auf den Master-Start.");
}
...
function joinExistingTest() {
  ...
  setRecordStatus(`Du bist Test ${state.testId} als ${roleName(state.role)} beigetreten.`);
...
function bindEvents() {
  ...
  addClick("masterStartButton", async () => {
    if (state.cameraRecording) await masterStopAll();
    else await masterStartAll();
  });
